...
sum_e = 0;
data_save=[];
s10=[0.013,0,0,0,000*pi/30,2.9*3000*pi/30];
...
[t1,s1]=runge_kutta4(@phase_1,s10,1e-5,t10,t1f,options);
...
value=s2(1)-10; %Stops when s1(11)=1e-3
isterminal=0; %Stop after the first event (=0 to get all the events)
direction=0; % No matter which direction (+ -> - or - -> +)
% direction=-1;
end
...
value=s3(1)-10; %Stops when s1(11)=1e-3
isterminal=0; %Stop after the first event (=0 to get all the events)
direction=0; % No matter which direction (+ -> - or - -> +)
% direction=-1;
end
